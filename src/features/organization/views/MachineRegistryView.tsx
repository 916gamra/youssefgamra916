  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 pt-2">
        <div>
          <h1 className="text-3xl font-bold text-slate-100 tracking-tight mb-2 flex items-center gap-3 uppercase">
            <Factory className="w-8 h-8 text-indigo-500" /> Machine Registry
          </h1>
          <p className="text-slate-400 text-lg font-bold opacity-80 uppercase tracking-widest text-[10px]">Digital Twin Hub: Register and monitor physical assets across sectors.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="bg-black/40 border border-white/10 rounded-xl pl-9 pr-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30 transition-all w-64 shadow-inner"
            />
          </div>
          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold uppercase tracking-widest text-[10px] transition-all shadow-lg shadow-indigo-500/20 shrink-0"
          >
      ...
      {filteredMachines.length === 0 ? (
         <div className="p-16 text-center border border-white/5 rounded-2xl bg-white/[0.02]">
            <Cpu className="w-16 h-16 text-slate-500 mx-auto mb-4 opacity-30" />
            <h3 className="text-xl font-bold text-slate-100 uppercase tracking-widest mb-2">Null Results Detected</h3>
            <p className="text-slate-400 text-sm font-medium">No assets matching your query or registry is empty.</p>
            <button
               onClick={() => setIsModalOpen(true)}
               className="mt-6 px-6 py-2 rounded-xl border border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/10 transition-colors uppercase tracking-widest text-[10px] font-bold"
            >
               + Sync First Machine
            </button>
         </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <AnimatePresence>
            {filteredMachines.map((machine, idx) => (
              ...
              >
                <GlassCard className="relative overflow-hidden group h-full flex flex-col hover:border-indigo-500/30 transition-all duration-300">
                  ...
                  <div className="mb-4 relative z-10">
                    <div className="flex justify-between items-start mb-3">
                      <span className="inline-block px-2.5 py-1 bg-white/5 border border-white/10 rounded-md text-[10px] font-mono font-bold text-indigo-400 tracking-wider shadow-sm">
                    ...
                    <h3 className="text-lg font-bold text-slate-100 leading-none mb-1.5 tracking-tight uppercase truncate">{machine.name}</h3>
                  ...
                  <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between text-[10px] font-bold text-slate-400 uppercase tracking-widest relative z-10">
              ...
              <div className="p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <input 
                        ...
                        className="titan-input font-mono text-indigo-400"
                      />
                    </div>
                  </div>
                  ...
                  <div className="pt-6 flex justify-end gap-4">
                    <button type="button" onClick={handleClose} className="px-6 py-2.5 rounded-xl border border-white/5 text-slate-400 hover:text-white transition-all font-bold text-[10px] uppercase tracking-widest text-[#94a3b8]">
                      Abort
                    </button>
                    <button type="submit" className="px-8 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-500/20 transition-all flex items-center gap-3">
