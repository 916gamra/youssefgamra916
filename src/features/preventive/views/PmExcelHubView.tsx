...
import { FileSpreadsheet, Download, Upload, Server } from 'lucide-react';
import { motion } from 'motion/react';

export function PmExcelHubView() {
  return (
      ...
      >
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-10">
          <div>
            ...
            <p className="text-slate-400 uppercase tracking-widest text-xs mt-4 font-semibold flex items-center gap-2">
          ...
          <div className="flex gap-4">
              <div className="bg-black/40 p-4 rounded-2xl border border-white/5 shadow-inner flex items-center gap-4 group hover:border-blue-500/30 transition-all cursor-default overflow-hidden relative">
                <div className="absolute inset-0 bg-blue-500/[0.02] opacity-0 group-hover:opacity-100 transition-opacity" />
                <Download className="w-5 h-5 text-blue-500 opacity-60" />
                <div>
                   <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest block">Format</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
           <div className="lg:col-span-1 space-y-6">
              <div className="titan-card p-6 bg-blue-500/[0.02] border-blue-500/20">
                 <h3 className="text-xs font-bold text-white mb-4 uppercase tracking-widest flex items-center gap-2">
                    <Server className="w-4 h-4 text-blue-400" /> Maintenance Systems
                 </h3>
                 <p className="text-[11px] text-[#8b9bb4] leading-relaxed">
                    Import master schedules and historical work orders. Ensure columns match the standard schema for successful synchronization.
                 </p>
              </div>
              
              <div className="titan-card p-6 bg-white/[0.01]">
                 <h4 className="text-[10px] font-bold text-white/40 mb-3 uppercase tracking-widest">Supported Entities</h4>
           ...
           <div className="lg:col-span-3">
              <div className="titan-card p-0 overflow-hidden bg-black/40 border-white/10 group min-h-[500px]">
                <div className="p-6 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                   <span className="text-[10px] font-bold text-white/60 uppercase tracking-widest">Data Management Terminal</span>
                ...
                <div className="p-8">
                  <ExcelManager portalId="PREVENTIVE" />
                </div>
              </div>
           </div>
        </div>
