...
export function FactoryExcelHubView() {
  return (
    <motion.div 
    ...
    >
      <header className="flex flex-col gap-2 mb-8 pt-2 border-b border-white/10 pb-6">
        <h1 className="text-3xl font-semibold text-[var(--text-bright)] tracking-tight flex items-center gap-3">
          <FileSpreadsheet className="w-8 h-8 text-indigo-400" /> Plant Excel Hub
        </h1>
        <p className="text-[var(--text-dim)] text-lg">
          Master data integration for Factory layout, production lines, and technical staff.
        </p>
      </header>

      <ExcelManager portalId="Factory" />
    </motion.div>
  );
}
