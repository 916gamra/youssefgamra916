              ...
              >
                <Dialog.Content asChild>
                  <motion.div
                    ...
                    className="w-full max-w-md bg-[var(--bg-base)]/80 backdrop-blur-2xl border border-[var(--glass-border)] shadow-2xl rounded-2xl overflow-hidden shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] relative"
                  >
                    ...
                    <div className="p-6">
                      ...
                      <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-1.5">
                          <label className="text-xs uppercase font-semibold tracking-wider text-[var(--text-dim)]">Target Item</label>
                          <select
                            value={stockId}
                            onChange={(e) => setStockId(e.target.value)}
                            disabled={isProcessing}
                            className="w-full bg-black/30 border border-[var(--glass-border)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-bright)] focus:outline-none focus:border-[var(--accent)] transition-colors appearance-none disabled:opacity-50"
                          >
                        ...
                        {/* Transaction Type */}
                        <div className="space-y-1.5">
                          <label className="text-xs uppercase font-semibold tracking-wider text-[var(--text-dim)]">Movement Type</label>
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                type === 'IN' 
                                  ...
                                  : "bg-black/30 border-[var(--glass-border)] text-[var(--text-dim)] hover:bg-white/5"
                              )}
                            >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          {/* Quantity */}
                          <div className="space-y-1.5">
                            <label className="text-xs uppercase font-semibold tracking-wider text-[var(--text-dim)] flex items-center justify-between">
                              <span>Quantity</span>
                              {selectedItem && <span className="text-[10px] text-[var(--accent)]">Avail: {selectedItem.quantityCurrent} {selectedItem.unit}</span>}
                            </label>
                            <input
                              type="number"
                              min="0.01"
                              ...
                              disabled={isProcessing}
                              placeholder="0.00"
                              className="w-full bg-black/30 border border-[var(--glass-border)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-bright)] focus:outline-none focus:border-[var(--accent)] transition-colors disabled:opacity-50"
                            />
                          </div>

                          {/* Performed By */}
                          <div className="space-y-1.5">
                            <label className="text-xs uppercase font-semibold tracking-wider text-[var(--text-dim)]">Technician</label>
                            <input
                              type="text"
                              required
                              value={performedBy}
                        ...
                        <div className="pt-4 flex justify-end gap-3">
                          <button
                            ...
                            onClick={onClose}
                            disabled={isProcessing}
                            className="px-5 py-2.5 rounded-xl border border-[var(--glass-border)] text-[var(--text-dim)] hover:text-[var(--text-bright)] hover:bg-white/5 transition-all font-medium text-sm disabled:opacity-50"
                          >
                            Cancel
                          </button>
                          <button
